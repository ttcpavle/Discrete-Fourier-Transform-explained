...
public class FFT {
    ...
    public static Complex[] directFT(Complex x[])
    {
        ...
        Complex W = Complex.exp(-2*Math.PI / (double) N); // (cos(-2pi/N), sin(-2pi/N)) which is e^(-2*pi*i/N)
    ...
    // inverse DFT from definition (similar to directFT but without '-' in exponent and with 1/N normalization)
    public static Complex[] inverseFT(Complex[] X) {
    	
    	int N = X.length;
        Complex[] x = new Complex[N];
        Complex W = Complex.exp(2*Math.PI / (double) N);
        Complex Wk = new Complex(1, 0);
    ...
    public static Complex[] recursiveFFT(Complex x[])
    {
        ...
        if(N==1)                                       // A length-1 vector is its own FT;
            ...
            int N2 = N >> 1; // division by 2

            Complex[] xe = new Complex[N2]; // even
            Complex[] xo = new Complex[N2]; // odd
            Complex[] X = new Complex[N];
    ...
    public static Complex[] iterativeFFT(Complex x[])
    {
        ...
        for(int k=0; k<r; k++) {
            for(int l=0; l<N; l+=2*step) {
                // Twiddle factors:
                Complex W = Complex.exp(-Math.PI / (double) step);
                Complex Wkn = new Complex(1, 0);
                for(int n=0; n<step; n++) {
                    int p = l + n;
                    int q = p + step;
                    if (q < N) {
                        Complex temp = Wkn.mul(X[q]);
                        X[q] = X[p].sub(temp);
                        X[p] = X[p].add(temp);
                    }
